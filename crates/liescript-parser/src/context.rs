...
use liescript_diagnostic::{
    diagnostic::{Diagnostic, DiagnosticBuilder, DiagnosticKind, Phase, Severity},
...
use liescript_lexical::{
    ...
    literal::Literal,
    symbol::Symbol,
    token::{Punctuated, Token, TokenSpan},
};
...
impl<'i> ParseContext<'i> {
    ...
    /// 期望符号
    pub fn expect_symbol(&mut self, symbol: Symbol) -> ParseResult<TokenSpan> {
        self.expect(
    ...
    /// 期望关键字
    pub fn expect_keyword(&mut self, keyword: Keyword) -> ParseResult<TokenSpan> {
        self.expect(
        ...
        F: FnOnce(&mut Self) -> ParseResult<T>,
    {
        ...
        match parse_fn(self) {
            ...
            Err(error) => {
                // 记录错误到新的诊断系统
                let diagnostic: Diagnostic = error.clone().into();
...
impl<'i> ParseContext<'i> {
    /// 创建Token不匹配错误
    pub fn create_token_mismatch_error(
        &self,
        expected: &str,
        found: &str,
        span: Span,
    ) -> ParseError {
        ParseError::token_mismatch(expected, found, span)
    }

    /// 创建括号不匹配错误
    pub fn create_bracket_mismatch_error(
        &self,
        open_bracket: &str,
        close_bracket: &str,
        span: Span,
    ) -> ParseError {
        ParseError::bracket_mismatch(open_bracket, close_bracket, span)
    }

    /// 创建操作符错误
    pub fn create_operator_error(&self, message: &str, span: Span) -> ParseError {
        ParseError::operator_error(message, span)
    }

    /// 创建表达式错误
    pub fn create_expression_error(&self, message: &str, span: Span) -> ParseError {
        ParseError::expression_error(message, span)
    }
}
