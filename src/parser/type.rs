...
/// 解析原生类型
fn parse_primitive_type(cx: &mut ParseContext, primitive: fn(TokenSpan) -> Primitive) -> ParseResult<Type> {
...
fn parse_path_type(cx: &mut ParseContext) -> ParseResult<Type> {
    ...
    let segment = crate::syntax::names::PathIdentSegment::Ident(ident.into());
    let type_segment = crate::syntax::names::TypePathSegment {
        ident: segment,
        args: None,
    };
    ...
    Ok(Type::Path(crate::syntax::TypePath { leading_colon, segments }))
}
...
fn parse_reference_type(cx: &mut ParseContext) -> ParseResult<Type> {
    ...
    Ok(Type::Reference(crate::syntax::types::ReferenceType {
        and_token,
        is_mut,
        ty: Box::new(ty),
    }))
...
fn parse_tuple_type(cx: &mut ParseContext) -> ParseResult<Type> {
    ...
    let mut punctuated_elems = crate::lexical::Punctuated::new();
    for elem in elems {
        punctuated_elems.push_last(elem);
    }
    
    Ok(Type::Tuple(crate::syntax::types::TupleType {
...
fn parse_array_or_slice_type(cx: &mut ParseContext) -> ParseResult<Type> {
    ...
    if cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::Semi))) {
        ...
        let len = super::expression::parse_expression(cx)?;
        let rbracket = cx.expect_symbol(Symbol::RBracket)?;
        
        Ok(Type::Array(crate::syntax::types::ArrayType {
        ...
        Ok(Type::Slice(crate::syntax::types::SliceType {
            bracket_token: crate::lexical::Bracket::new(lbracket, rbracket),
            elem: Box::new(elem),
        }))
    }
...
fn parse_never_type(cx: &mut ParseContext) -> ParseResult<Type> {
    ...
    Ok(Type::Never(crate::syntax::types::NeverType { bang_token }))
}

/// 解析推断类型
fn parse_inferred_type(cx: &mut ParseContext) -> ParseResult<Type> {
    let underscore_token = cx.expect_symbol(Symbol::Underscore)?;
    Ok(Type::Inferred(crate::syntax::types::InferredType { underscore_token }))
}