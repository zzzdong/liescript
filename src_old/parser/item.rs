...
use crate::{
    ...
    syntax::items::{
        ConstItem, EnumItem, FunctionItem, Item, ModuleItem, StaticItem, StructItem, TypeAliasItem,
        UseItem,
    },
};
...
fn parse_function_item(cx: &mut ParseContext) -> ParseResult<FunctionItem> {
    ...
    Ok(FunctionItem {
        fn_token,
        name: crate::diagnostic::Spanned::new(name.value.into_ident(), name.span),
        generics,
        params: crate::syntax::items::FunctionParams {
            ...
            params: {
                ...
                for (pat, colon, ty) in params {
                    punctuated.push_last(crate::syntax::items::FunctionParam {
                        pattern: pat,
        ...
        return_type,
        body: crate::syntax::items::FunctionBody {
...
fn parse_struct_item(cx: &mut ParseContext) -> ParseResult<StructItem> {
    ...
    let fields = if cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::LBrace))) {
        ...
        while !cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::RBrace))) && !cx.is_eof() {
            ...
            let colon_token = cx.expect_symbol(Symbol::Colon)?;
            let field_type = parse_type(cx)?;
            
            named_fields.push(crate::syntax::items::NamedField {
                name: crate::diagnostic::Spanned::new(field_name.value.into_ident(), field_name.span),
                colon_token,
        ...
        let rbrace = cx.expect_symbol(Symbol::RBrace)?;
        crate::syntax::items::StructFields::Named(crate::syntax::items::NamedFields {
            brace_token: crate::lexical::Brace::new(lbrace, rbrace),
        ...
        while !cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::RParen))) && !cx.is_eof() {
            let field_type = parse_type(cx)?;
            tuple_fields.push(crate::syntax::items::TupleField {
                visibility: None, // TODO: 支持可见性
                ty: Box::new(field_type),
            });
        ...
        let rparen = cx.expect_symbol(Symbol::RParen)?;
        crate::syntax::items::StructFields::Tuple(crate::syntax::items::TupleFields {
            paren_token: crate::lexical::Paren::new(lparen, rparen),
            visibility: None, // TODO: 支持可见性
            fields: {
                let mut punctuated = crate::lexical::Punctuated::new();
        ...
        // 单元结构体
        crate::syntax::items::StructFields::Unit
    };
...
fn parse_enum_item(cx: &mut ParseContext) -> ParseResult<EnumItem> {
    ...
    while !cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::RBrace))) && !cx.is_eof() {
        ...
        let variant_fields = if cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::LBrace))) {
            ...
            let rbrace = cx.expect_symbol(Symbol::RBrace)?;
            Some(crate::syntax::items::EnumVariantFields::Named(crate::syntax::items::NamedFields {
                brace_token: crate::lexical::Brace::new(lbrace, rbrace),
                visibility: None,
                fields: {
                    let mut punctuated = crate::lexical::Punctuated::new();
            ...
            let rparen = cx.expect_symbol(Symbol::RParen)?;
            Some(crate::syntax::items::EnumVariantFields::Tuple(crate::syntax::items::TupleFields {
                paren_token: crate::lexical::Paren::new(lparen, rparen),
                visibility: None,
                fields: {
                    let mut punctuated = crate::lexical::Punctuated::new();
        ...
        variants.push(crate::syntax::items::EnumVariant {
            name: crate::diagnostic::Spanned::new(variant_name.value.into_ident(), variant_name.span),
...
fn parse_module_item(cx: &mut ParseContext) -> ParseResult<ModuleItem> {
    ...
    let content = if cx.next_is(|t: &Token| matches!(t, Token::Symbol(Symbol::LBrace))) {
        ...
        let rbrace = cx.expect_symbol(Symbol::RBrace)?;
        Some(crate::syntax::items::ModuleContent {
            brace_token: crate::lexical::Brace::new(lbrace, rbrace),
            items,
        })
    } else {
...
fn parse_use_item(cx: &mut ParseContext) -> ParseResult<UseItem> {
    ...
    let path = parse_simple_path(cx)?;
    let semi_token = cx.expect_symbol(Symbol::Semi)?;

    // TODO: 实现完整的use树解析
    Ok(UseItem {
        use_token,
        tree: crate::syntax::items::UseTree::Path(path),
        semi_token,
    })
}
